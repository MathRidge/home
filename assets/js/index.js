...
function bindPremiumMobileSelection() {
  ...
  document.addEventListener("pointerdown", event => {
    ...
    if (!event.target.closest(".small-link, .jump-link, .hotspot, .room-spot, .cabin-banner-return-home, .certificate-banner-return-cabin, .relic-banner-return-cabin, .cabin-tab, .reset-progress-btn, #sendNoteButton, #modalActions button")) {
      clearStageSelection();
      mobileConfirm()?.clear?.();
    }
  }, { passive: true });
